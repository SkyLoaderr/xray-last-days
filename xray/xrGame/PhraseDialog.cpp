...
void CPhraseDialog::AddPhrase	(CUIXml* pXml, XML_NODE* phrase_node, const shared_str& phrase_id, const shared_str& prev_phrase_id)
{
	...
	int answer_time			= pXml->ReadInt(phrase_node, "answer_time", 0, 0);
	if (answer_time > 0)
	{
		ph->SetAnswerTime(answer_time);
		ph->m_time_up_phrase = pXml->Read(phrase_node, "time_up_phrase", 0, "");
		R_ASSERT2(ph->m_time_up_phrase.size(), make_string("Can't find node 'time_up_phrase' in phrase '%s', dialog '%s'", phrase_id.c_str(), GetDialogID().c_str()));

		XML_NODE* next_phrase_node		= pXml->NavigateToNodeWithAttribute("phrase", "id", ph->GetTimeUpPhrase());
		R_ASSERT2						(next_phrase_node, ph->GetTimeUpPhrase());
		AddPhrase						(pXml, next_phrase_node, ph->GetTimeUpPhrase(), phrase_id);
	}
