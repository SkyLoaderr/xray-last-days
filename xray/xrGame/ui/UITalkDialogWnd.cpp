...
CUITalkDialogWnd::CUITalkDialogWnd()
	:	m_pNameTextFont		(NULL), m_iAnswerTime(-1), m_iAnswerTimeElapsed(-1)
{
	m_ClickedQuestionID = "";
	mechanic_mode = false;
...
//	AttachChild					(&UIOthersIcon);
	...
	//UIDialogFrameBottom			= xr_new<CUIStatic>();
	//UIDialogFrameBottom->SetAutoDelete(true);
	//AttachChild					(UIDialogFrameBottom);
	//CUIXmlInit::InitStatic		(*m_uiXml, "frame_bottom", 0, UIDialogFrameBottom);

	//основной фрейм диалога
	UIDialogFrameTop			= xr_new<CUIStatic>();
	UIDialogFrameTop->SetAutoDelete(true);
	...
	//Прогресс бар
	UIProgressBar				= xr_new<CUIProgressBar>();
	UIProgressBar->SetAutoDelete(true);
	AttachChild					(UIProgressBar);
	CUIXmlInit::InitProgressBar	(*m_uiXml, "progress_bar", 0, UIProgressBar);
	UIProgressBar->SetWindowName("---UIProgressBar");
	UIProgressBar->Show			(false);
	UIProgressBar->SetProgressPosAgressive(UIProgressBar->GetRange_max());
...
void CUITalkDialogWnd::OnQuestionClicked(CUIWindow* w, void*)
{
	...
	m_iAnswerTime = m_iAnswerTimeElapsed = -1;
	UIProgressBar->Show(false);
...
void CUITalkDialogWnd::AddTimeUpQuestion(LPCSTR str, LPCSTR value, int answer_time, bool b_finalizer)
{
	m_TimeUpPhraseID = value;
	m_iAnswerTime = m_iAnswerTimeElapsed = _max(answer_time, 1);

	UIProgressBar->Show(true);
	UIProgressBar->SetProgressPosAgressive(UIProgressBar->GetRange_max());
}

void CUITalkDialogWnd::UpdateProgressBar()
{
	if (m_iAnswerTimeElapsed <= 0) return;

	m_iAnswerTimeElapsed -= Device.dwTimeDelta;

	float pos = float(m_iAnswerTimeElapsed) / float(m_iAnswerTime);
	UIProgressBar->SetProgressPos(pos);

	if (m_iAnswerTimeElapsed <= 0)
	{
		UIProgressBar->Show(false);
		m_ClickedQuestionID = m_TimeUpPhraseID;
		GetMessageTarget()->SendMessage(this, TALK_DIALOG_QUESTION_CLICKED);
	}
}
