	...
	}else
	{
		if(m_pCurrentDialog->IsWeSpeaking(m_pOurDialogManager))
		{
			...
			{			
				...
				{
					CPhrase* phrase = *it;
					// if phrase used for time up answer
					CPhrase* last_phrase = m_pCurrentDialog->GetPhrase(m_pCurrentDialog->GetLastPhraseID());
					if (!xr_strcmp(last_phrase->GetTimeUpPhrase(), phrase->GetID()))
					{
						LPCSTR text = m_pCurrentDialog->GetPhraseText(phrase->GetID());
						UITalkDialogWnd->AddTimeUpQuestion(CStringTable().translate(text).c_str(), phrase->GetID().c_str(), last_phrase->AnswerTime(), phrase->IsFinalizer());
						continue;
					}
...
void CUITalkWnd::Update()
{
	...
	UITalkDialogWnd->UpdateButtonsLayout(b_disable_break, m_pOthersInvOwner->IsTradeEnabled());
	UITalkDialogWnd->UpdateProgressBar();
