...
void	CRenderTarget::phase_combine	()
{
	...
	if (!_menu_pp)
	{
		...
		Fvector4	envclr;		
		if (ps_r2_ls_flags_ext.test(R2FLAGEXT_SHOC_ENV_CALC))
	...
	if (1)	
	{
		...
		CEnvDescriptorMixer& envdesc	= *g_pGamePersistent->Environment().CurrentEnv;
		Fvector4	tnmpdefog			= { envdesc.tonemap_defog.x,	envdesc.tonemap_defog.y,	envdesc.tonemap_defog.z,		0 };
		...
		RCache.set_c				("tonemap_defog",	tnmpdefog	);
		Fvector3					dof;
...
//.		RCache.set_c				("dof_params",	ps_r2_dof.x, ps_r2_dof.y, ps_r2_dof.z, ps_r2_dof_sky);
		...
		RCache.set_c				("effects_intensity", ps_r2_ls_tonemap_intensity, ps_r2_ls_vignette_intensity, ps_r2_ls_abberation_amount, ps_r2_ls_lensflares_intensity);
